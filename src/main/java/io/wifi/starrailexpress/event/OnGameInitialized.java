 ...
 */
public interface OnGameInitialized {
     ...
     */
    Event<OnGameInitialized> EVENT = createArrayBacked(OnGameInitialized.class,
            listeners -> (sl) -> {
                for (OnGameInitialized listener : listeners) {
                    listener.onGameInitialized(sl);
                }
            });
     ...
     */
    void onGameInitialized(ServerLevel serverLevel);
}
