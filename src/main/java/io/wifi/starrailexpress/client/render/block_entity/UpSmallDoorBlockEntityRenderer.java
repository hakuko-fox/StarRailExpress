...
public class UpSmallDoorBlockEntityRenderer extends AnimatableBlockEntityRenderer<UpSmallDoorBlockEntity> {
    ...
    public static LayerDefinition getTexturedModelData() {
        ...
        modelPartData.addOrReplaceChild("UpDoor", CubeListBuilder.create().texOffs(28, 56).addBox(-8.0F, -2.0F, -1.0F, 16.0F, 2.0F, 2.0F, new CubeDeformation(0.0F))
    ...
    @Override
    public void render(UpSmallDoorBlockEntity entity, float tickDelta, PoseStack matrices, MultiBufferSource vertexConsumers, int light, int overlay) {
    ...
    @Override
    public void setAngles(UpSmallDoorBlockEntity entity, float animationProgress) {
        this.root().getAllParts().forEach(ModelPart::resetPose);
        this.part.setRotation(0, entity.getYaw() * Mth.DEG_TO_RAD, 0);
        this.animate(entity.state, entity.isOpen() ? SmallDoorAnimations.UP_OPEN : SmallDoorAnimations.UP_CLOSE, animationProgress);
    }

    @Override
    public ResourceLocation getTexture(UpSmallDoorBlockEntity entity, float tickDelta) {
        return this.texture;
    }
