...
/**
 * 自定义职业服务端同步（走统一的内容同步通道，见 {@link ContentSyncServer}）。
 *
 * <p>
 * 职业配置通常比其它几个系统大得多，压缩下发的收益也最明显。
 */
public final class CustomRoleServerNetwork {

    private CustomRoleServerNetwork() {
    }

    /** 配置变更后调用：丢弃缓存并给所有在线玩家重新握手。 */
    public static void syncToAllPlayers(MinecraftServer server) {
        ContentSyncServer.invalidate(ContentChannel.CUSTOM_ROLE);
