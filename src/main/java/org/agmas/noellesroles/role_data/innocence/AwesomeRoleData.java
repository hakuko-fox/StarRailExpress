...
public class AwesomeRoleData extends SimpleRoleData {
     ...
     * 构造函数
     */
    public AwesomeRoleData(RoleDataContext context) {
        super(context);
    }
            ...
            for (var player : players) {
                if (GameUtils.isPlayerAliveAndSurvival(player)) {
                    if (player.distanceToSqr(victim) <= 25) {
                        AwesomeRoleData component = RoleData.getNullable(AwesomeRoleData.class, player);
                        if (component != null) {
