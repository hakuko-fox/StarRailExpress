...
public class DIORoleData extends SimpleRoleData {
     ...
     * 构造函数
     */
    public DIORoleData(RoleDataContext context) {
        super(context);
    }
    ...
    static {
        AllowPlayerDeath.EVENT.register((player, resourceLocation) -> {
            if (SREGameWorldComponent.KEY.get(player.level()).isRole(player, ModRoles.DIO)) {
                DIORoleData dio = RoleData.getNullable(DIORoleData.class, player);
                if (dio == null)
                    return true;
