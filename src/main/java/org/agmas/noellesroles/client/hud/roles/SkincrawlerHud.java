...
public class SkincrawlerHud {
    public static void register() {
        RoleHudRenderCallback.EVENT.register(ModRoles.SKINCRAWLER_ID, (context, deltaTracker) -> {
            ...
            var comp = RoleData.getOptional(SkincrawlerRoleData.class, client.player);
            ...
            if (comp.get().stealCooldown > 0) {
                int sec = (comp.get().stealCooldown + 19) / 20;
            ...
            // 显示抵挡剩余次数
            Component blockText = Component.translatable("hud.noellesroles.skincrawler.block_charges", comp.get().blockCharges)
                    .withStyle(comp.get().blockCharges > 0 ? ChatFormatting.GREEN : ChatFormatting.RED);
