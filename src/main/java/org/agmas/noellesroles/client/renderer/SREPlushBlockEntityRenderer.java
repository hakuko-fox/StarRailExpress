...
public class SREPlushBlockEntityRenderer<T extends BlockEntity> implements BlockEntityRenderer<T> {
    ...
    @Override
    public void render(T blockEntity, float delta, PoseStack poseStack, MultiBufferSource consumer, int light, int overlay) {
        poseStack.pushPose();
        var squish = blockEntity instanceof SREPlushBlockEntity plushie ? plushie.squash : 0;
        var lastSquish = squish * 3;
        var squash = (float)Math.pow((double)1.0F - (double)1.0F / ((double)1.0F + Mth.lerp((double)delta, lastSquish, squish)), (double)2.0F);
        ...
        this.renderer.getModelRenderer().renderModel(poseStack.last(), consumer.getBuffer(ItemBlockRenderTypes.getRenderType(state, false)), state, bakedModel, 255F, 255F, 255F, light, overlay);
        poseStack.popPose();
    }
