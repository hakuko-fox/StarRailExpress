 ...
 */
public class RabbitDisguiseRenderer {
    private static final Map<UUID, Rabbit> RABBITS = new HashMap<>();

    // ==== 猪头几何（PigModel：head 枢轴在模型 (0,12,-6)，8×8×8 的头，前方再伸出 1px 猪吻） ====
    // 模型坐标 -> 实体根坐标：y = 1.501 - my/16，向前 = -mz/16。
    /** 猪头旋转枢轴距脚底的高度：1.501 - 12/16 */
    private static final float HEAD_PIVOT_Y = 0.371F;
    /** 猪头旋转枢轴相对身体中心向前的偏移：6/16 */
    private static final float HEAD_PIVOT_FORWARD = 0.0625F;
    /** 枢轴到猪头正面的距离：8/16 */
    private static final float FACE_FORWARD = 0.175F;
    /** 脸部半宽/半高（猪头正面恰好 8×8 像素） */
    private static final float FACE_HALF = 0.0625F;
    ...
    public static boolean shouldDisguise(AbstractClientPlayer player) {
        return (RoleUtils.isPlayerTheModifier(player, NRModifiers.RABBIT_SHAPE));
    }
     ...
     */
    private static Vec3 rabbitEyeOffset(AbstractClientPlayer player, float tickDelta) {
    ...
    public static boolean render(AbstractClientPlayer player, float yaw, float tickDelta,
        ...
        Rabbit rabbit = getRabbit(player);
        if (rabbit == null) {
            return false;
        }
        ...
        if (rabbit.tickCount != player.tickCount) {
            rabbit.walkAnimation.update(player.walkAnimation.speed(), 1.0f);
            rabbit.tickCount = player.tickCount;
        }
        rabbit.setPos(player.getX(), player.getY(), player.getZ());
        rabbit.xo = player.xo;
        rabbit.yo = player.yo;
        rabbit.zo = player.zo;
        ...
        rabbit.setXRot(0.0F);
        rabbit.xRotO = 0.0F;
        rabbit.setInvisible(player.isInvisible());
        rabbit.hurtTime = player.hurtTime;
        ...
        poseStack.pushPose();
        if (firstPersonSelf) {
            Vec3 offset = rabbitEyeOffset(player, tickDelta);
            poseStack.translate(-offset.x, 0.0, -offset.z);
        }

        EntityRenderer<? super Rabbit> renderer = minecraft.getEntityRenderDispatcher().getRenderer(rabbit);
        renderer.render(rabbit, yaw, tickDelta, poseStack, bufferSource, packedLight);
     ...
     *
     * <p>
     * 脸部必须贴在猪头这块骨骼上，因此要沿着 PigModel 的层级走一遍：抬到枢轴高度 →
    ...
    private static Rabbit getRabbit(AbstractClientPlayer player) {
        Rabbit rabbit = RABBITS.get(player.getUUID());
        if (rabbit == null || rabbit.level() != player.level()) {
            rabbit = EntityType.RABBIT.create(player.level());
            if (rabbit != null) {
                RABBITS.put(player.getUUID(), rabbit);
            }
        }
        return rabbit;
    }
