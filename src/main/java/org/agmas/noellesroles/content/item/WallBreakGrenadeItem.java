...
import org.agmas.noellesroles.content.entity.WallBreakGrenadeEntity;
import org.agmas.noellesroles.init.ModEntities;
import org.jetbrains.annotations.NotNull;

/**
 * 破墙弹物品：右键蓄力投掷，蓄力时间越长投掷越远。命中墙壁后拆除半径 2 的方块，5 秒后自动恢复（持久化，重启不丢失）。带 NBT 的方块不拆除。
 */
public class WallBreakGrenadeItem extends Item {

    public static final int MAX_CHARGE_TIME = 20; // 最大蓄力时间（ticks），同 GrenadeItem

    public WallBreakGrenadeItem(Properties settings) {
        super(settings);
    }
    ...
    public void releaseUsing(ItemStack stack, Level world, LivingEntity user, int remainingUseTicks) {
        ...
        if (!world.isClientSide) {
            ...
            WallBreakGrenadeEntity grenade = new WallBreakGrenadeEntity(ModEntities.WALL_BREAK_GRENADE, world);
