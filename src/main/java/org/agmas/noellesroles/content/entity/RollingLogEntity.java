...
/**
 * 滚木实体：沿指定方向滚动，碾死沿途玩家，撞墙或寿命耗尽后碎裂。
 * 渲染为一根横躺的原版橡木原木，参考 {@link RollingStoneEntity} 实现。
 */
public class RollingLogEntity extends Entity {
    ...
    public static RollingLogEntity spawn(ServerLevel level, Vec3 pos, Direction dir) {
    ...
    public void tick() {
        ...
        if (this.tickCount % 5 == 0) {
            level.playSound(null, this.blockPosition(), SoundEvents.WOOD_HIT, SoundSource.BLOCKS, 1.6F, 0.5F);
        }
    ...
    private void shatter(ServerLevel level) {
        ...
        level.playSound(null, this.blockPosition(), SoundEvents.WOOD_BREAK, SoundSource.BLOCKS, 1.8F, 0.6F);
    }
}
