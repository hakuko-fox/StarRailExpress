  ...
  public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
    ...
    dispatcher.register(Commands.literal(roleDetailsCommandRoot)
        .then(Commands.literal("role")
            .then(Commands.argument("role", RoleArgumentType.create()).executes((ctx) -> showRoleDetails(ctx,
  ...
  private static int showRole(CommandContext<CommandSourceStack> context, int page) {
    ...
    if (!subList.isEmpty()) {
      ...
      for (Object entry : subList) {
        if (entry instanceof SRERole role) {
          ...
          message.append(
              buildElementText(RoleUtils.getRoleOrModifierNameWithColor(modifier), modifier.identifier(), status,
                  false));
        }
        message.append("\n");
      }
    ...
    if (maxTotalPages > 0) {
      ...
      buttons.append(Component.literal("[")
          ...
          .append(Component.literal("]"))
          .withStyle(style -> style.withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/listRoles 1"))
      ...
      if (page > 1) {
        buttons.append(Component.literal("[")
            ...
            .withStyle(style -> style
                .withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/listRoles " + (page - 1)))
                .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
      ...
      buttons.append(Component.literal("[")
          ...
          .append(Component.literal("]"))
          .withStyle(style -> style
              .withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/listRoles " + maxTotalPages))
              .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
  ...
  private static MutableComponent buildElementText(Component name, ResourceLocation identifier, Component status,
    ...
    return Component.empty().append(name)
        ...
        .append(status).withStyle(style -> style
            .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                Component.translatable("commands.listroles.detail.click_to_show").withStyle(ChatFormatting.AQUA)))
            .withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND,
                "/" + roleDetailsCommandRoot + " " + (isRole ? "role" : "modifier") + " " + identifier)));
  }
