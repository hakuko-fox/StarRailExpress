...
- **结构**:
  ...
  - `reset` — 重置为自动计算
- **用途**: 覆盖自动计算的职业分配数量
- **适用范围**: 常规阵营分配、VTuber/HK VTuber、职业轮选与自荐模式。轮选/自荐在自动模式下至少生成一名杀手，但手动设置 `killer 0` 会保留为 0。
- **限制**: 固定队伍或固定职业玩法（如传统模式无中立、躲猫猫无中立及小游戏专用队伍）仍遵循该模式规则。禁用、地图条件、关联职业及可用职业不足也会限制实际人数。
...
### `nametag:backfill` — 补发称号
- **权限**: `2`
- **结构**: `[targets]` (Players)
- **用途**: 按在线玩家已加载的统计和连续记录补发符合条件的称号；支持玩家名、选择器和 `@a`
- **限制**: 不补发无法从现有数据验证的单局事件称号或管理员专属称号
