...
$menu = Read-Utf8 'src/main/java/org/agmas/noellesroles/client/screen/VtuberPlayerSelectScreen.java'
Assert-Contains $menu 'button.active = ready' '冷卻時必須停用玩家頭像。'
Assert-Contains $menu 'Long.toString(seconds)' '玩家頭像缺少冷卻倒數。'
...
foreach ($language in @('zh_tw', 'zh_cn', 'en_us')) {
    ...
    Assert-Contains $intro['info.screen.roleid.kana'] '一次性小刀' "$language 佳奈說明缺少一次性小刀。"
    Assert-Contains $intro['info.screen.roleid.meowlen'] '立即結束本次作答' "$language 貓倫說明缺少立即結束。"
    Assert-Contains $intro['info.screen.roleid.baiyu'] '右下角技能區' "$language 白御說明缺少標記 HUD。"
}
